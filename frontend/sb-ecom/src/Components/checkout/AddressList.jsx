...
import { setSelectedCheckoutAddress } from '../../store/action';
...
const AddressList = ({ addresses, setSelectedAddress, setOpenAddressModal,setOpenDeleteModal}) => {
    ...
    const dispatch = useDispatch();
      const selectedCheckoutAddress = useSelector(
        (state) => state.auth.selectedCheckoutAddress
    );

    const handleAddressSelection = (address) => {
        dispatch(setSelectedCheckoutAddress(address));
    };
        ...
        <div className='space-y-4'>
            {addresses.map((address) => (
                <div key={address.addressId}
                    ...
                    className={`p-4 rounded-md border cursor-pointer relative ${selectedCheckoutAddress ?.addressId === address.addressId
                        ? "bg-green-500"
                        : "bg-white"
                        }`}
                >
                    <div className='flex items-start'>
                        <div className='space-y-1'>
                            <div className='flex items-center'>
                                ...
                                {selectedCheckoutAddress ?.addressId === address.addressId && (
